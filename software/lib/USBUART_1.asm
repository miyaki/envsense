...
_USBUART_1_bGetRxCount:
    RAM_PROLOGUE RAM_USE_CLASS_3
	RAM_SETPAGE_IDX >USBUART_1_EndpointAPIStatus ; Set the IDX_PP to the right page
...
USBUART_1_Write_Start:
	...
	xor     [USBUART_1_EPDataToggle+2], USB_CNT_TOGGLE

	mov     A, [X+CNTLEN]                  ;Get the count
    mov     reg[USBUART_1_EP2CNT0], A   ; Write it into the register
	mov     A, [USBUART_1_EPDataToggle+2]  ; Retrieve the saved toggle (MSB)
...
; load zero length dummy packet to EP2
	...
	mov     A, [USBUART_1_EPDataToggle+2]  ; Get The Data toggle for EP
	mov     reg[USBUART_1_EP2CNT1], A      ; Write it into the register
...
; Enable EP 3
    RAM_SETPAGE_IDX >USBUART_1_EndpointAPIStatus
    mov     [USBUART_1_EndpointAPIStatus+3], NO_EVENT_PENDING ; For the API
	mov     reg[USBUART_1_EP3CNT0], 64 
	M8C_SetBank1
    mov     A, reg[TMP_DR2]
...
_USBUART_1_SendStateNotify:
	...
	xor     [USBUART_1_EPDataToggle+1], USB_CNT_TOGGLE

    mov     reg[USBUART_1_EP1CNT0], 10  ; Write it into the register
	mov     A, [USBUART_1_EPDataToggle+1]  ; Retrieve the saved toggle (MSB)
